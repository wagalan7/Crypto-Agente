    ...
    <div className="bg-gray-900/60 border border-gray-800 rounded-xl overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Tabs */}
          <button
            onClick={() => { setTab('google'); setFetched(false); setCampaigns([]); setError('') }}
            className={`text-[11px] px-3 py-1 rounded-lg border transition-all
              ${tab === 'google' ? 'border-yellow-600 bg-yellow-900/30 text-yellow-300' : 'border-gray-700 text-gray-500 hover:text-gray-300'}`}>
            G Google Ads
          </button>
          <button
            onClick={() => { setTab('facebook'); setFetched(false); setFbInsights(null); setError('') }}
            className={`text-[11px] px-3 py-1 rounded-lg border transition-all
              ${tab === 'facebook' ? 'border-blue-600 bg-blue-900/30 text-blue-300' : 'border-gray-700 text-gray-500 hover:text-gray-300'}`}>
            𝕗 Facebook
          </button>
        </div>
      </div>
        ...
        {tab === 'google' && fetched && campaigns.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: 'Impressões', value: fmt(totals.impressions), color: 'text-blue-400' },
                { label: 'Cliques',    value: fmt(totals.clicks),      color: 'text-violet-400' },
                { label: 'CTR médio',  value: avgCtr + '%',            color: 'text-emerald-400' },
                { label: 'Gasto total',value: fmtBRL(totals.cost),     color: 'text-amber-400' },
              ].map(k => (
                ...
                <tbody>
                  {campaigns.map((c, i) => (
                    <tr key={i} className={`border-t border-gray-800 ${i % 2 === 0 ? 'bg-gray-900/40' : ''}`}>
                      <td className="px-3 py-2 text-gray-200 max-w-[180px] truncate font-medium">{c.campaign_name || '—'}</td>
